...
public struct ProjectSummary: Identifiable, Decodable, Equatable, Sendable, Hashable {
    ...
    /// Creates a placeholder ProjectSummary for navigation purposes
    public static func placeholder(id: Int) -> ProjectSummary {
        // Safely construct URL using URLComponents
        ...
        guard let webUrl = components.url else {
            // This should never happen with hardcoded valid components
            fatalError("Failed to construct placeholder URL for project \(id)")
        }

        return ProjectSummary(
            id: id,
            name: "Loading...",
            pathWithNamespace: "loading/\(id)",
            namespaceName: nil,
            description: nil,
            starCount: 0,
            forksCount: 0,
